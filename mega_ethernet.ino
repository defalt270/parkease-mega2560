...
IPAddress ip(192,168,137,200); //IP address for arduino.

char server[] = "192.168.0.24"; //IP address of computer.

int port = 80; //server port

int interrupt=0; //Variable to control the iterations of void loop().

String mod="testing_moto"; //Variable to be written on the database.
...
void httpRequest()
{
  if (client.connect(server, port)) 
  {
    ...
    client.print(String("GET ") + "/ethernet/tryjson.php/" + " HTTP/1.1\r\n" + "Host: " + server + "\r\n" + "Connection: close\r\n\r\n"); //GET request for server response.
...
void loop() 
{
  ...
  if(interrupt==0)
  {
      ...
      client.print("GET /ethernet/info.php?"); //GET request to write data to the database.
